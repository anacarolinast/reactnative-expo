...
const CityCard = ({ city_name, description, condition_slug, temp }) => {
  ...
  const gradientColors =
    route.name === "Search"
      ? ["rgba(0, 0, 0, 0)", "rgba(0, 0, 0, 0)"] // Cores transparentes para a página de busca
      : ["rgba(26, 26, 140, 0.4)", "rgba(26, 26, 140, 0)"]; // Cores padrão

  return (
    <TouchableOpacity
      onPress={() => handleCityCard(city_name)}
      style={{
        width: route.name === "Search" ? "90%" : "",
      }}
    >
      <LinearGradient
        ...
        <View style={{ flex: 1 }}>
          <Text
            style={{
              ...
              fontWeight: "bold",
              color: "white",
              textAlign: route.name === "Search" ? "center" : "",
            }}
          >
            {limitedCityName}
          </Text>
