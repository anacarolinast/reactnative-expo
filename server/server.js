...
app.get('/weather', async (req, res) => {
   try {
       const response = await axios.get(`https://api.hgbrasil.com/weather?key=4e3f71ab&lat=-8.11208&lon=-35.0154&user_ip=remote`);
...
app.get('/weather/cities', async (req, res) => {
    try {
        ...
        const weatherDataPromises = randomCities.map(city => {
            ...
            return axios.get(`https://api.hgbrasil.com/weather?key=4e3f71ab&city_name=${city}`);
        });
    ...
    app.get('/weather/city', async (req, res) => {
        try {
            ...
            const response = await axios.get(`https://api.hgbrasil.com/weather?key=4e3f71ab&city_name=${cityName}`);
            res.json(response.data);
        } catch (error) {
            res.status(500).json({ error: 'Erro ao obter dados meteorológicos.' });
        }
